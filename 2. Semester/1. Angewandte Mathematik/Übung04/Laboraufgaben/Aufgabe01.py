...
#unsere werte
u1 = 0.0 #start position
u2 = 50.0
v1 = 0.1 #start geschwindigkeit
v2 = 0.1
m1 = 1
m2 = 1
...
while 0 <= u1 < 55:
    ...
    #position berechnen
    p, tv = RailwayTrack(u1)
    R = array([[tv[0], -tv[1]], 
               [tv[1],  tv[0]]])

    #zeichne das ding an position p und rotation R aus der matrix oben
    DrawCart(p, R)
    #formatted string mit variable im text .2f float 2 nachkommastellen

    #plt.pause(0.01)#pausiert bild, ohne das funktioniert es irgendwie nicht, geht keine animation mit camera

    a1 = g * tv[1]   # beschleunigung tangential zur Schiene
    v1 = v1 + (a1 * dt)  # neue Geschwindigkeit
    u1 = u1 + (v1 * dt)  # neue Position

    p, tv = RailwayTrack(u2)
    R = array([[tv[0], -tv[1]], 
               [tv[1],  tv[0]]])
    ...
    a2 = g * tv[1]   # beschleunigung tangential zur Schiene
    v2 = v2 + (a2 * dt)  # neue Geschwindigkeit
    u2 = u2 + (v2 * dt)  # neue Position


    if u1 + 1 > u2:
        v12neu = (m1*v1 + m2*v1) / (m1 + m2)
        v1 = v12neu
        v2 = v12neu
        u2 = u1 + 1.001
